...
- tool_search_regex("file|directory") --> finds tools mentioning "file" or "directory"`,
    ...
    execute(args: { pattern: string; category?: "all" | "builtin" | "mcp" | "plugin" }, ctx: Tool.Context) {
      return Effect.promise(async () => {
        ...
        if (results.length === 0) {
          return {
            ...
            metadata: { pattern: args.pattern, count: 0, tools: [] as string[], displayOutput: "No tools found" },
